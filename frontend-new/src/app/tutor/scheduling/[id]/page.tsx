...
export default function SchedulingPage() {
  ...
  const [desiredMinutesFromTutee, setDesiredMinutesFromTutee] = useState<
    number | null
  >(null);
  const [scheduling, setScheduling] = useState(false);
  const [availabilityOptions, setAvailabilityOptions] = useState<string[]>([]);
  const [allowedMask, setAllowedMask] = useState<{
    [k: string]: Array<{ start: string; end: string }>;
  }>({});
  ...
  const loadJobData = async () => {
    try {
      ...
      const resp = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/tutor/jobs/${jobId}`,
        {
          headers: { Authorization: `Bearer ${session?.access_token ?? ""}` },
        }
      );
      if (!resp.ok) {
        setError("Job not found or not permitted");
        return;
      }
      ...
      const a = (jobData?.tutee_availability || {}) as {
        [k: string]: string[];
      };
  ...
  const handleScheduleSession = async () => {
    try {
      ...
      if (
        desiredMinutesFromTutee !== null &&
        mins !== desiredMinutesFromTutee
      ) {
        throw new Error(
          `Session must be exactly ${desiredMinutesFromTutee} minutes long.`
        );
      }
      ...
      const r = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/tutor/jobs/${jobId}/schedule`,
        {
          method: "POST",
          headers: {
          ...
          await apiService.sendCancellationNotification(
              ...
              "",
            {
              subject: `${job?.tutoring_opportunity?.subject_name || ""} • ${
                ...
                job?.tutoring_opportunity?.subject_grade || ""
              }`.trim(),
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1">
                      ...
                      {job.tutoring_opportunity?.subject_type} • Grade{" "}
                      {job.tutoring_opportunity?.subject_grade}
                    </p>
                  </div>
                  <div></div>
                  <div>
                    ...
                    <p className="mt-1">
                      {job.tutoring_opportunity?.session_location}
                    </p>
                  </div>
                    ...
                    <div className="mt-2">
                      ...
                      job.tutoring_opportunity?.availability_end_time ? (
                        ...
                        <p className="text-sm text-gray-500">
                          {job.tutoring_opportunity?.availability_formatted ||
                            "Not specified"}
                        </p>
                      )}
                    </div>
            ...
            <div>
              ...
              <div className="space-y-6">
                <div className="border rounded p-3">
                  <div className="mb-2 text-sm font-medium text-gray-700">
                    Choose a time within the tutee's availability
                    {desiredMinutesFromTutee
                      ? ` — exactly ${desiredMinutesFromTutee} minutes`
                      : ""}
                  </div>
                  <TwoWeekTimeGrid
                    value={dateSelection}
              ...
              {desiredMinutesFromTutee && (
                <div className="mt-4 text-sm text-gray-700">
                  Session duration requested by tutee:{" "}
                  <span className="font-medium">
                    {desiredMinutesFromTutee} minutes
                  </span>
                </div>
              )}
              ...
              <div className="mb-6 p-4 bg-yellow-50 rounded-md">
                ...
                <p className="text-sm text-yellow-700">
                  You must select a single continuous block exactly equal to the
                  tutee's requested duration
                  {desiredMinutesFromTutee
                    ? ` (${desiredMinutesFromTutee} minutes)`
                    : ""}
                  , and it must be within the tutee's availability (green
                  regions).
                </p>
              </div>
