  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (scheduled === 'success') {
        setSuccessMessage('Session scheduled successfully! The tutee has been notified.');
      } else if (cancelled === 'true') {
        setSuccessMessage('Job cancelled successfully. The opportunity has been returned to the board.');
      } else if (completed === 'success') {
        setSuccessMessage('Session completed successfully! Your volunteer hours have been added to your account.');
      }
    }
  }, []);
  ...
  useEffect(() => {
    const fetchTutorData = async () => {
      ...
      if (userRole === null) {
        ...
        setTimeout(() => {
          if (userRole === null) {
            console.log('Dashboard: Role determination timeout, redirecting to login');
            router.push('/auth/login');
          }
        }, 2000);
        return;
      ...
      try {
        ...
        try {
          ...
          if (jobsError) {
            ...
            if (jobs && jobs.length > 0) {
              const opportunityIds = jobs.map(job => job.opportunity_id).filter(Boolean);
              ...
              if (opportunityIds.length > 0) {
                ...
                if (oppError) {
                  ...
                  const opportunityMap = (opportunities || []).reduce((map, opp) => {
                    map[opp.id] = opp;
                    return map;
                  }, {});
                  ...
                  const transformedJobs = jobs.map(job => {
                    ...
                    return {
                      ...
                      tutoring_opportunity: {
                        ...opportunity,
                        tutee_name: `${opportunity.tutee_first_name || ''} ${opportunity.tutee_last_name || ''}`.trim(),
        ...
        try {
          ...
          if (approvalsError) {
            console.error('Error fetching subject approvals:', approvalsError.message);
            setSubjectApprovals([]);
          } else {
            // If we have approvals, fetch the related subjects separately
            if (approvals && approvals.length > 0) {
              const subjectIds = approvals.map(approval => approval.subject_id).filter(Boolean);
  ...
  const handleCancelJob = async (jobId: string, opportunityId: string) => {
    ...
    try {
      ...
      try {
        ...
        console.log('Email sending failed (expected in development mode) - continuing with cancellation');
      }
      ...
      setSuccessMessage('Tutoring job cancelled successfully. The opportunity has been returned to the board.');
      ...
      <div className="p-6 bg-gray-50 min-h-full">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {tutorData?.school?.name} • {tutorData?.status === 'active' ? 'Active Tutor' : 'Pending Approval'}
          </p>
        </div>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg font-medium text-gray-900">
                      {subjectApprovals.filter(s => s.status === 'approved').length}
                    </dd>
                  </dl>
                </div>
              </div>
        ...
        {/* Active Jobs */}
        <div className="mb-8">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Active Tutoring Jobs</h3>
          {activeJobs.length > 0 ? (
                        ...
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            <div className="text-sm font-medium text-gray-900">
                              {job.tutoring_opportunity.subject} - {job.tutoring_opportunity.tutee_name}
                            </div>
                            <div className="text-sm text-gray-500">
                              {job.tutoring_opportunity.location_preference} • {job.status}
                            </div>
                            {job.scheduled_time ? (
                              ...
                              <div className="text-xs text-gray-400 mt-1">
                                Available: {job.tutoring_opportunity.availability && (
                                  <span className="inline-flex items-center">
                                    <span className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-xs">
                                      {formatTime(job.tutoring_opportunity.availability.split(' - ')[0])}
                                    </span>
                                    <svg className="w-3 h-3 mx-1 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                                    </svg>
                                    <span className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-xs">
                                      {formatTime(job.tutoring_opportunity.availability.split(' - ')[1])}
                                    </span>
                                  </span>
                                )}
                              </div>
                      ...
                      {expandedJobs.has(job.id) && (
                        <div className="mt-4 pt-4 border-t border-gray-200">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              <h4 className="text-sm font-medium text-gray-700 mb-2">Tutee Details</h4>
                              <div className="text-sm text-gray-600 space-y-1">
                                <p><span className="font-medium">Name:</span> {job.tutoring_opportunity.tutee_name}</p>
                                <p><span className="font-medium">Email:</span> {job.tutoring_opportunity.tutee_email}</p>
                              </div>
                            </div>
                            <div>
                              <h4 className="text-sm font-medium text-gray-700 mb-2">Session Details</h4>
                              <div className="text-sm text-gray-600 space-y-1">
                                <p><span className="font-medium">Subject:</span> {job.tutoring_opportunity.subject}</p>
                                <p><span className="font-medium">Location:</span> {job.tutoring_opportunity.location_preference}</p>
                                {job.scheduled_time ? (
                                  <p><span className="font-medium">Scheduled Time:</span> {formatTime(job.scheduled_time)}</p>
                                ) : (
                                  <p><span className="font-medium">Availability:</span> {job.tutoring_opportunity.availability && (
                                    <span className="inline-flex items-center">
                                      <span className="px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full text-xs">
                                        {formatTime(job.tutoring_opportunity.availability.split(' - ')[0])}
                                      </span>
                          ...
                          <div className="mt-4 flex justify-end space-x-2">
                            ...
                            {job.status === 'scheduled' && (
                              <button 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  router.push(`/sessions/${job.id}/complete`);
                                }}
        ...
        {/* Subject Approvals */}
        <div>
          <h3 className="text-lg font-medium text-gray-900 mb-4">Subject Approvals</h3>
          {subjectApprovals.length > 0 ? (
            ...
            <div className="text-center py-8 bg-white rounded-lg shadow">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">No subject approvals</h3>
              <p className="mt-1 text-sm text-gray-500">
                Contact your school administrator to get approved for subjects you can tutor.
              </p>
            </div>
          )}
        </div>
